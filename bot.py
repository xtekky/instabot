...
class InstagramBot:
	...
	def UnfollowUser(self, user):
		...
		if (button and button[0].text != "Obserwuj"):
			if (len(button) > 2):
				button[1].click()
			else:
				button[0].click()
	...
	def Login(self):
		...
		self.Wait(1,2)
		self.driver.find_elements_by_xpath("/html/body/div[4]/div/div/button[1]")[0].click()
		self.Wait(3,4)
		self.driver.find_element_by_name("username").send_keys(self.username)
		self.Wait(1,2)
		self.driver.find_element_by_name("password").send_keys(self.password)
		self.Wait(1,2)
		self.driver.find_elements_by_xpath("//div[contains(text(), 'Zaloguj')]")[0].click()
	...
	def FollowFollowers(self):
		...
		numoffollowers = 1000000000
		amount = int(input("How many to follow?"))
		...
		while (k <= amount):
			...
			if (button) and (button[0].text == "Obserwuj") and (name not in self.doNotFollowList):
				self.Wait(30,35)
				button[0].click()
	...
	def TagOptions(self):
		...
		for i in range(amount):
			self.Wait(40,45)
			name = self.driver.find_element_by_xpath('/html/body/div[6]/div[3]/div/article/div/div[2]/div/div/div[1]/div/header/div[2]/div[1]/div[1]/div/span/a').text
			# Liking the post
			if liketag:
				self.Wait(1,2)
				likebutton = self.driver.find_element_by_xpath("/html/body/div[6]/div[3]/div/article/div/div[2]/div/div/div[2]/section[1]/span[1]")
				liketext = likebutton.find_element_by_xpath(".//*[name()='svg']").get_attribute('aria-label')
				if liketext == "Lubię to!":
					likebutton.click()
					print("[{}] *liked* post by {}".format(i+1, name))
			# Commenting on the post
			if commenttag:
				self.Wait(1,2)
				temp = self.driver.find_element_by_xpath('/html/body/div[6]/div[3]/div/article/div/div[2]/div/div/div[2]/section[3]/div/form')
				...
				self.Wait(1,2)
				self.driver.find_element_by_xpath('/html/body/div[6]/div[3]/div/article/div/div[2]/div/div/div[2]/section[3]/div/form/button').click()
